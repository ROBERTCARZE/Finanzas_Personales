    ...
    card.innerHTML = `
      <div class="card-top">
        ...
        <div>
          ...
          <div class="card-sub">${g.targetDate ? 'Meta: ' + formatDate(g.targetDate) : ''}${linkedAccount ? ' · 💳 ' + escapeHtml(linkedAccount.name) : ' · meta virtual'}</div>
        </div>
...
// ---------- Aporte rápido ----------
// Si la meta está vinculada a una cuenta real (Módulo 4), el aporte
// mueve dinero de verdad: resta el saldo de la cuenta y lo suma a la
// meta, en una sola operación atómica. Si no está vinculada, es un
// aporte "virtual" que solo incrementa el ahorro de la meta.
async function addContribution(goal) {
  ...
  const accountRef = doc(db, 'users', uid, 'accounts', goal.linkedAccountId);
  try {
    await runTransaction(db, async (t) => {
      ...
      const currentSaved = Number(goalSnap.data()?.currentAmount) || 0;

      t.update(accountRef, { balance: currentBalance - amount });
      t.update(goalRef, { currentAmount: currentSaved + amount });
    });
  } catch (err) {
    console.error(err);
