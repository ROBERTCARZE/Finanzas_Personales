...
// ---------- Exportar a CSV (respaldo local de tus datos) ----------
function exportCsv() {
  if (transactions.length === 0) { alert('No hay transacciones para exportar.'); return; }

  const headers = ['Folio', 'Fecha', 'Tipo', 'Categoría', 'Cuenta', 'Cuenta Destino', 'Monto', 'Notas', 'Hashtags'];
  const rows = [...transactions].sort((a, b) => a.folio - b.folio).map((tx) => {
    const category = tx.categoryId ? (categories.find((c) => c.id === tx.categoryId)?.name || '') : '';
    const account = accounts.find((a) => a.id === tx.accountId)?.name || '';
    const destination = tx.destinationAccountId ? (accounts.find((a) => a.id === tx.destinationAccountId)?.name || '') : '';
    return [
      tx.folio,
      tx.date,
  ...
  link.href = url;
  link.download = `transacciones_${todayISO()}.csv`;
  link.click();
...
function attachEventListeners() {
  document.getElementById('newTxBtn').addEventListener('click', () => {
    if (accounts.length === 0) {
      alert('Todavía no tienes ninguna cuenta creada. Ve al módulo "Cuentas y Activos" y crea al menos una antes de registrar una transacción.');
      window.location.href = 'cuentas.html?new=account';
      return;
    }
    openDrawer();
  });
