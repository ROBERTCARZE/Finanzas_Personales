...
async function loadCycleTip() {
  const snap = await getDocs(query(collection(db, 'users', uid, 'cycleLogs'), orderBy('startDate')));
  ...
  const cycleDay = ((diffDays % avgCycleLength) + avgCycleLength) % avgCycleLength + 1;
