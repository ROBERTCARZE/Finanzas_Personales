...
// ---------- Alertas ----------
function renderAlerts(flujoEconomico) {
  const stack = document.getElementById('alertsStack');
  stack.innerHTML = '';
  flujoEconomico.forEach((v, i) => {
    if (v < 0) {
      const el = document.createElement('div');
      el.className = 'alert-item danger';
      el.textContent = `Flujo de caja económico negativo en ${MONTH_LABELS[i]} ${selectedYear}: ${formatMoney(v)}.`;
