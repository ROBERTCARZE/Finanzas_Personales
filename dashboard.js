...
async function loadLifestyleSettings() {
  const cycleSnap = await getDoc(doc(db, 'users', uid, 'settings', 'cycle'));
  if (cycleSnap.exists()) cycleSettings = cycleSnap.data();
...
function renderLifestyle() {
  ...
  if (cycleSettings?.lastPeriodStart) {
    const start = new Date(cycleSettings.lastPeriodStart + 'T00:00:00');
    const today = new Date();
    const diffDays = Math.floor((today - start) / (1000 * 60 * 60 * 24));
    const cycleDay = ((diffDays % cycleSettings.avgCycleLength) + cycleSettings.avgCycleLength) % cycleSettings.avgCycleLength + 1;
    const ovulationDay = cycleSettings.avgCycleLength - 14;
    let phase;
    if (cycleDay <= cycleSettings.avgPeriodLength) phase = 'Menstrual';
